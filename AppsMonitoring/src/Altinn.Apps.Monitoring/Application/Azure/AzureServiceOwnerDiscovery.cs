...
internal sealed class AzureServiceOwnerDiscovery(AzureClients clients, AzureServiceOwnerResources serviceOwnerResources) : IServiceOwnerDiscovery
{
    ...
    public async ValueTask<IReadOnlyList<ServiceOwner>> Discover(CancellationToken cancellationToken)
    {
        ...
        await Parallel.ForEachAsync(_armClient.GetSubscriptions().GetAllAsync(), async (subscription, cancellationToken) => 
        {
